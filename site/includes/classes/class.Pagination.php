...
class Paginator{
	...
	public function __construct($total=0,$mid_range=7,$ipp_array=array(10,25,50,100,"All")) {
		...
		if($this->num_pages > 10) {
			...
			$this->return .= ($this->current_page > 1 And $this->total_items >= 10) ? "<li> <a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=".($this->current_page-1)."&ipp=$this->items_per_page$this->querystring\">Previous</a></li> ":"<span class=\"inactive\" href=\"#\">Previous</span> ";
			...
			for($i=1;$i<=$this->num_pages;$i++) {
				...
				if($i==1 Or $i==$this->num_pages Or in_array($i,$this->range)) $this->return .= ($i == $this->current_page And $this->items_per_page != "All") ? "<li><a title=\"Go to page $i of $this->num_pages\" class=\"current\" href=\"#\">$i</a></li> \n":"<li><a class=\"paginate\" title=\"Go to page $i of $this->num_pages\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a></li> \n";
				if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= "  ";
			}
			$this->return .= (($this->current_page < $this->num_pages And $this->total_items >= 10) And ($this->items_per_page != "All") And $this->current_page > 0) ? "<li><a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=".($this->current_page+1)."&ipp=$this->items_per_page$this->querystring\">Next</a></li>\n":"<span class=\"inactive\" href=\"#\">Next</span>\n";
			$this->return .= ($this->items_per_page == "All") ? "<li><a class=\"current\" style=\"margin-left:10px\" href=\"#\">All</a></li> \n":"<li><a class=\"paginate\" style=\"margin-left:10px\" href=\"$_SERVER[PHP_SELF]?page=1&ipp=All$this->querystring\">All</a></li> \n";
		} else	{
			for($i=1;$i<=$this->num_pages;$i++) {
				$this->return .= ($i == $this->current_page) ? "<li class='active'><a class=\"current\" href=\"#\">$i</a></li> ":"<li><a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a> </li>";
			}
	...
	public function display_jump_menu() {
		...
		return "<span class=\"paginate\">Jump to Page:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
